...
function PackageDetailsScreen({route, navigation}) {
    ...
    const fetchPackage = async () => {
      try {
        let res = await axios.get(
          `https://travelbuddy-1-4ja0.onrender.com/TravelBuddy/packages/slug/${slug}`,
        );

        if (!res.data) {
          res = await axios.get(
            `https://travelbuddy-1-4ja0.onrender.com/TravelBuddy/packages/${slug}`,
          );
        }
  ...
  const addToCart = async () => {
    try {
      const cartItem = {
        ...
        image:
          pkg.images?.sightseeing?.[0] ||
          pkg.images?.hotel?.[0] ||
          pkg.images?.restaurant?.[0],
  ...
  const bookPackage = async () => {
    try {
      ...
      // Navigate directly to cart where payment can be made
      navigation.navigate('Cart');
    } catch (error) {
      console.error('Error booking package:', error);
      Alert.alert('Error', 'Failed to book package');
    }
  };
  ...
  const getTravelIcon = mode => {
    ...
    if (m?.includes('by air'))
      return <Icon name="plane" size={16} color="#333" />;
    if (m?.includes('by train'))
      return <Icon name="train" size={16} color="#333" />;
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
        <TouchableOpacity
          style={[
            styles.button,
            inCart ? styles.inCartButton : styles.wishlistButton,
          ]}
          onPress={addToCart}
          disabled={inCart}>
